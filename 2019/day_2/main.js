...
class AOC2019DayTwo {
    constructor() {
        ...
        this.path = '/opt/project/2019/day_2/input.txt';
        this.input = null;
        this.subscriptions = [];
    }
    ;
    ...
    run() {
        ...
        this.subscriptions.push(this.assignment1(), this.assignment2());
    }
    testAssignment1() {
        const testCases = [
    ...
    assignment1() {
        ...
        return this.parsedInput$().pipe(operators_1.map(data => this.parseProgram(data)[0])).subscribe(data => console.log('Day 2, Assignment 1 answer = ' + data));
    }
    assignment2() {
        ...
        const options$ = rxjs_1.from(options);
        return options$.pipe(operators_1.mergeMap(option => {
            return this.parsedInput$().pipe(operators_1.map(data => {
                data[1] = option[0];
                data[2] = option[1];
                return this.parseProgram(data);
            }), operators_1.skipWhile(data => data[0] !== 19690720));
        })).subscribe(data => {
    ...
    parsedInput$() {
        return this.reader.lines$.pipe(operators_1.map(data => data[0].split(',').map(Number)));
    }
}
exports.AOC2019DayTwo = AOC2019DayTwo;
